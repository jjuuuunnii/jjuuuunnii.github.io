...
const PORTFOLIO_DATA = {
  ...
  hero: {
    ...
    description:
      "IoT 공유 자전거 스타트업에서 1년 9개월간 일하며 백엔드 개발팀장으로 3인 팀을 이끌고, 운영 중인 4개 플랫폼의 서버를 개발·운영했습니다. 장애 위험과 협업 문제가 드러날 때마다 기술만 바꾸지 않고 구조를 고쳤습니다. 이후 두 번의 공동창업에서는 PM으로 제품을 출시하고 데이터로 검증해 종료까지 판단했으며, 첫 창업에서는 백엔드 개발도 전담했습니다.",
    ...
    achievements: [
      {
        ...
        solution:
          "결과를 나중에 앱으로 보내는 SSE 구조를 팀에 먼저 제안하고, 운영과 병행해 2주간 SSE·Kafka를 공부해 구현했습니다. 장비 통신에만 쓰던 Kafka는 서버 간 비동기 통신으로 넓혔습니다.",
        result:
      ...
      },
      {
        ...
        result:
          "조회 API를 늘릴 때 대여·반납 명령 코드를 건드리지 않아도 되는 구조가 됐고, 두 영역을 나눠 개발할 수 있게 됐습니다.",
        details: [
      ...
      {
        version: "",
        title: "사용자 문의로 알던 장비 명령 장애를 1분 안에 알림으로 인지",
        subtitle: "실패 유형 13종 정의 · 웹훅·SSE·푸시 3경로 전파",
        problem:
          "IoT 장비 명령이 실패해도 사용자 문의가 들어와야 알 수 있어, 인지까지 1시간 넘게 걸리곤 했습니다.",
        solution:
          "장비 명령 실패를 유형별로 정의하고, 실패가 나면 바로 알림이 가도록 전파 경로를 만들었습니다.",
        result:
          "장비 명령 장애를 사용자 문의보다 먼저 알림으로 받아, 인지 시간이 1시간 이상에서 1분 이내로 줄었습니다.",
        details: [
          "실패 유형 13종 정의",
  ...
  ventures: {
    intro: {
      ...
      lesson:
        "발켄에서는 기술을 고르는 이유는 끝까지 물었지만, 이 사업이 정말 풀어야 할 문제인지는 묻지 않았습니다. 두 번의 공동창업에서는 PM으로 그 질문을 직접 맡았습니다. 포게티에서는 만들고 나서야 물었고, OFF STAGE에서는 출시 전에 합격선을 먼저 정했습니다.",
      delta: {
        from: { value: "약 3개월", label: "포게티 · 착수부터 실패를 알아차리기까지 (개발 포함)" },
        to: { value: "19일", label: "OFF STAGE · 런칭부터 종료 판단까지" },
      },
      takeaway:
        "재는 기준은 다르지만, 차이는 출시 전에 합격선과 계측을 두었는지에서 나왔습니다.",
    },
    program: {
      name: "아산나눔재단 아산 두어스",
        ...
        sections: [
          {
            ...
            tone: "solution",
            items: [
              "PM으로 PRD를 3판까지 고치고, North Star KPI(AI 전화 후 7일 안에 일정을 다시 등록하는 비율)를 정했습니다.",
          ...
          },
          {
            ...
            items: [
              ...
              "표본과 질문 방식에 한계가 있어 판결은 아니지만, '망각'이 핵심 문제라는 전제를 의심할 강한 사전 신호로 봤습니다.",
        ...
        sections: [
          {
            ...
            items: [
              ...
              "런칭 전에 합격선 5개(초대장 수락 60%, 저장 후 전원 승인 70% 등)를 가안으로 세웠습니다.",
            ],
          },
          {
            label: "측정 체계",
          ...
          },
          {
            ...
            items: [
              ...
              "합격선 5개 중 초대장 수락·전원 승인은 통과, 첫 세션 경험은 미달이었고, 30일 재방문·유료 전환은 기간이 짧아 판정할 수 없었습니다.",
              "문제 공감은 확인했지만 단체사진을 함께 보정할 일 자체가 자주 생기지 않는다고 보고, 런칭 19일 만에 종료를 결정했습니다.",
            ],
          },
        ],
  ...
  projects: [
    {
      ...
      highlights: [
        "JPQL에 IS NULL 조건을 두어, 값이 없는 필터는 건너뛰는 선택적 검색 쿼리 작성",
